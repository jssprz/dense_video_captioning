...
class SentenceLengthLoss(nn.Module):
    ...
    def forward(self, logits, targets, lens=None, rewards=None):
        if (lens is None) and (rewards is None):
            ...
            mask = (
                torch.cat([l.repeat(l) for l in lens], dim=0)
                .unsqueeze(1)
                .to(logits.device)
            )
            wighted_log_probs = torch.reciprocal(mask ** self.beta) * torch.log_softmax(
                logits, dim=1
            )
            ...
            mask2 = (
                torch.cat([r.repeat(l) for l, r in zip(lens, rewards)], dim=0)
                .unsqueeze(1)
                .to(logits.device)
            )
            # mask = torch.max(torch.ones_like(mask1), mask1**self.beta - mask2**self.beta)
            wighted_log_probs = (
                torch.reciprocal(mask1 ** self.beta)
                * mask2
    ...
    def forward(
        ...
        for n in range(bs):
            for i in range(gt_caps_count[n]):
                l1.append(
                    pred_captions[n, i, : gt_cap_lens[n, i]].reshape(
                        -1, caps_vocab_size
                    )
                )
                ...
                l3.append(
                    pred_pos_seq[n, i, : gt_cap_lens[n, i]].reshape(-1, pos_vocab_size)
                )
        ...
        if truncate_prog_at is not None:
            ...
            pred_program = pred_program[:, :truncate_prog_at].reshape(
                -1, progs_vocab_size
            )
            ...
            pred_program = torch.cat(
                [pred_program[n, : gt_prog_len[n]] for n in range(bs)], dim=0
            )

            # straighten the target captions (remove the part of the pad) and then flatten it
            # (total_len_of_programs)
            gt_program = torch.cat(
                [gt_program[n, : gt_prog_len[n]] for n in range(bs)], dim=0
            )
        if self.config.programer_iou_reward:
            iou_reward = temp_iou(pred_intervals, gt_intervals, gt_caps_count)
            prog_loss = self.programer_loss(
                pred_program, gt_program, gt_prog_len, iou_reward
