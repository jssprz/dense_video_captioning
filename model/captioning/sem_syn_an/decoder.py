...
class SemSynANDecoder(nn.Module):
    ...
    def forward_fn(self, v_feats, v_pool, s_tags, pos_emb, teacher_forcing_p=.5, gt_captions=None):
        ...
        v_sem_h = torch.zeros(batch_size, self.h_size)
        v_sem_c = torch.zeros(batch_size, self.h_size)

        v_syn_h = torch.zeros(batch_size, self.h_size)
        v_syn_c = torch.zeros(batch_size, self.h_size)

        se_sy_h = torch.zeros(batch_size, self.h_size)
        se_sy_c = torch.zeros(batch_size, self.h_size)
        ...
        if not self.training:
            ...
            # (batch_size x out_seq_length x output_size), none
            # print('\n'.join([str(o.size()) for o in outputs]))
            # print('\n'.join([str(w.size()) for w in words]))
            return torch.cat([o.unsqueeze(1) for o in outputs], dim=1).contiguous(), torch.cat([w.unsqueeze(1) for w in words], dim=1).contiguous()
