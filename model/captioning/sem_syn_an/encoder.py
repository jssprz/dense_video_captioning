...
from model.embeddings.visual_syntactic import VisualEncoding
...
class SCNEncoder(nn.Module):
    def __init__(
        ...
        i3d_feature_size,
        eco_feature_size,
        res_eco_features_size,
        n_tags,
        hidden_size,
        global_tagger_hidden_size,
        specific_tagger_hidden_size,
        vis_syn_embedd_space_size=512,
        pretrained_vis_syn_embedd_path="",
        input_dropout_p=0.2,
        rnn_dropout_p=0.5,
        n_layers=1,
        bidirectional=False,
        ...
        self.pos_model = VisualEncoding(
            in_size=res_eco_features_size,
            hidden_sizes=[2048, 1024],
            out_size=vis_syn_embedd_space_size,
            visual_norm=True,
            dropout_p=0.5,
            have_last_bn=True,
            pretrained_model_path=pretrained_vis_syn_embedd_path,
        )
    ...
    def forward(
        ...
        v_concat = v_concat.view(
            batch_size, -1, self.cnn_feature_size + self.c3d_feature_size
        )

        s_globals = torch.cat(
            (tags_globals, torch.softmax(cnn_sem_globals, dim=1)), dim=1
        )
