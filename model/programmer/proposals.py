...
class ProposalsEncoder(nn.Module):
    def __init__(
        ...
        self.prop_s_rnn_1 = nn.LSTMCell(
            input_size=proposals_tagger_config.rnn_h_size,
            hidden_size=proposals_tagger_config.rnn_h_size,
        )
    ...
    def excecute_instruction(self, a_id, p, q):
        ...
        vix_2_adv = torch.tensor(
            list(
                set(
                    vix_2_skip[
                        (q[vix_2_skip] > self.prop_e_0_pos[vix_2_skip]).nonzero(
                            as_tuple=True
                        )[0]
                    ].tolist()
                    + vix_2_adv[
                        (q[vix_2_adv] > self.prop_e_0_pos[vix_2_adv]).nonzero(
                            as_tuple=True
                        )[0]
                    ].tolist()
                )
    ...
    def prop_s_rnn_0_step(self, vix_2_skip, p, v_feats, feats_count):
        ...
        fidx_from = torch.min(
            p[vix_2_skip], feats_count[vix_2_skip] - self.future_steps
        )
        ...
        if len(attn_mask):
            ...
            v_p[attn_mask] = self.v_p_attn(
                torch.stack(
                    [
                        v_feats[vix, ff:ft, :]
                        for vix, ff, ft in zip(
                            vix_2_attn, fidx_from[attn_mask], fidx_to[attn_mask]
                        )
                    ]
                ),
        ...
        if len(fill_mask):
            ...
            v_p[fill_mask] = v_feats[
                vix_2_fill, torch.min(p[vix_2_fill], feats_count[vix_2_fill]), :
            ]

        # compute a recurrent step of the first layer (forward) of START MODULE
        (
            self.prop_s_h_0[vix_2_skip, :],
            self.prop_s_c_0[vix_2_skip, :],
        ) = self.prop_s_rnn_0(
            v_p, (self.prop_s_h_0[vix_2_skip, :], self.prop_s_c_0[vix_2_skip, :])
        )

    def prop_s_back_rnn_0_enc(
        self, vix_2_dscr, p, v_feats, feats_count, max_back_steps
    ):
        ...
        _, (self.prop_s_back_h_0[vix_2_dscr], _) = self.prop_s_back_rnn_0(
            sub_v_feats_packed
        )
    ...
    def prop_s_rnn_1_step(self, vix_2_dscr):
        ...
        (
            self.prop_s_h_1[vix_2_dscr, :],
            self.prop_s_c_1[vix_2_dscr, :],
        ) = self.prop_s_rnn_1(
            self.prop_s_h_0[vix_2_dscr, :],
            (
                self.prop_s_h_1[vix_2_dscr, :],
                self.prop_s_c_1[vix_2_dscr, :],
            ),
    ...
    def prop_e_rnn_0_step(self, vix_2_adv, q, v_feats, feats_count):
        ...
        if len(fill_mask):
            ...
            v_q[fill_mask] = v_feats[
                vix_2_fill, torch.min(q[vix_2_fill], feats_count[vix_2_fill]), :
            ]

        # compute a recurrent step of the first layer (forward) of END MODULE
        (
            self.prop_e_h_0[vix_2_adv, :],
            self.prop_e_c_0[vix_2_adv, :],
        ) = self.prop_e_rnn_0(
            v_q, (self.prop_e_h_0[vix_2_adv, :], self.prop_e_c_0[vix_2_adv, :])
        )

    def prop_e_back_rnn_0_enc(
        self, vix_2_dscr, p, q, v_feats, feats_count, max_back_steps
    ):
        ...
        _, (self.prop_e_back_h_0[vix_2_dscr], _) = self.prop_e_back_rnn_0(
            sub_v_feats_packed
        )
    ...
    def prop_e_rnn_1_step(self, vix_2_dscr):
        ...
        (
            self.prop_e_h_1[vix_2_dscr, :],
            self.prop_e_c_1[vix_2_dscr, :],
        ) = self.prop_e_rnn_1(
            self.prop_e_h_0[vix_2_dscr, :],
            (
                self.prop_e_h_1[vix_2_dscr, :],
                self.prop_e_c_1[vix_2_dscr, :],
            ),
    ...
    def train_val_step(
        ...
        vix_2_dscr = vix_2_dscr[caps_count[vix_2_dscr] < prop_logits_s.size(1)]
        ...
        if len(vix_2_dscr) > 0:
            ...
            self.prop_s_back_rnn_0_enc(
                vix_2_dscr, p, v_feats, feats_count, max_back_steps
            )
            ...
            prop_logits_s[vix_2_dscr, caps_count[vix_2_dscr], :] = self.prop_enc_s(
                torch.cat(
                    (self.prop_s_h_1[vix_2_dscr], self.prop_s_back_h_0[vix_2_dscr]),
                    dim=-1,
                )
            )[0]

            # END MODULE (back direction)
            self.prop_e_back_rnn_0_enc(
                vix_2_dscr, p, q, v_feats, feats_count, max_back_steps
            )
            ...
            prop_logits_e[vix_2_dscr, caps_count[vix_2_dscr], :] = self.prop_enc_e(
                torch.cat(
                    (self.prop_e_h_1[vix_2_dscr], self.prop_e_back_h_0[vix_2_dscr]),
                    dim=-1,
                )
            )[0]

        return self.prop_s_h_0, self.prop_e_h_0
    ...
    def dense_step(self, a_id, p, q, v_feats, feats_count, max_back_steps=10):
        ...
        # END MODULE (back direction)
        self.prop_e_back_rnn_0_enc(
            vix_2_enc, p, q, v_feats, feats_count, max_back_steps
        )
        # END MODULE (2nd layer)
        ...
        return vix_2_dscr, self.prop_s_h_1[vix_2_enc], self.prop_e_h_1[vix_2_enc]
