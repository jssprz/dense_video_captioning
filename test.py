...
if __name__ == "__main__":
    ...
    if tester_config.truncate_at < truncate_at:
        print(
            f" The model is trained for generating {truncate_at} long programs, but {tester_config.truncate_at} long is going to be used"
        )
        truncate_at = tester_config.truncate_at
    elif tester_config.truncate_at >= truncate_at:
        print(
            f" The model is trained for generating {truncate_at} long programs only. You need to train the model for longer programs"
        )

    if tester_config.max_caps < max_caps:
        print(
            f" The model is trained for generating until {max_caps} captions, but {tester_config.max_caps} are goint to be generated"
        )
        max_caps = tester_config.max_caps
    elif tester_config.max_caps >= max_caps:
        print(
            f" The model is trained for generating {max_caps} long programs only. You need to train the model for longer programs"
        )
    ...
    with torch.no_grad():
        ...
        for i, (vidx, fps) in enumerate(zip(test_vidxs, test_fps)):
            video_feats = [
                torch.from_numpy(cnn_feats[vidx]).unsqueeze(0),
                torch.from_numpy(c3d_feats[vidx]).unsqueeze(0),
            ]
            ...
            (
                ...
                gt_pos=None,
                gt_intervals=None,
                max_prog=truncate_at,
                max_caps=max_caps,
                max_cap=tester_config.max_words,
                max_chunks=truncate_at,
            )
            ...
            # print(f'  program: {p}')
            print(
                f" program-probs-avg: {torch.mean(torch.softmax(prog_logits, dim=-1), dim=-2)}"
            )
            programs.append(p)
            ...
            for i in range(caps_count):
                ...
                interval = [
                    float(tstamps[int(intervals[0, i, 0])]),
                    float(tstamps[int(intervals[0, i, 1])]),
                ]
